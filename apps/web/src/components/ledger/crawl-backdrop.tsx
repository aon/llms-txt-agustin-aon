...
/** The home's paper is a live crawl: every dot is a URL, the frontier grows breadth-first and leans toward the pointer, and a finished crawl fades before the next seed. */
export function CrawlBackdrop() {
  ...
  useEffect(() => {
    ...
    const stopFit = fitCanvas(canvas, (w, h, d) => {
      ...
      crawl.resize(w, h);
      if (still) {
        for (let k = 0; k < 900; k++) crawl.tick(k * TICK_S, null);
        draw(0);
      }
    });
    if (still)
